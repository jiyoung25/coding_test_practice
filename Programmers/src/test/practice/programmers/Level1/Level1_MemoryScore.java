package test.practice.programmers.Level1;

import java.util.HashMap;
import java.util.Map;

/*
 * https://school.programmers.co.kr/learn/courses/30/lessons/176963
 * 
 * 문제>>
 * 사진들을 보며 추억에 젖어 있던 루는 사진별로 추억 점수를 매길려고 합니다.
 * 사진 속에 나오는 인물의 그리움 점수를 모두 합산한 값이 해당 사진의 추억 점수가 됩니다.
 * 예를 들어 사진 속 인물의 이름이 ["may", "kein", "kain"]이고 각 인물의 그리움 점수가 [5점, 10점, 1점]일 때
 * 해당 사진의 추억 점수는 16(5 + 10 + 1)점이 됩니다.
 * 다른 사진 속 인물의 이름이 ["kali", "mari", "don", "tony"]이고
 * ["kali", "mari", "don"]의 그리움 점수가 각각 [11점, 1점, 55점]]이고,
 * "tony"는 그리움 점수가 없을 때, 이 사진의 추억 점수는 3명의 그리움 점수를 합한 67(11 + 1 + 55)점입니다.
 * 그리워하는 사람의 이름을 담은 문자열 배열 name, 각 사람별 그리움 점수를 담은 정수 배열 yearning, 각 사진에 찍힌 인물의 이름을 담은 이차원 문자열 배열 photo가 매개변수로 주어질 때,
 * 사진들의 추억 점수를 photo에 주어진 순서대로 배열에 담아 return하는 solution 함수를 완성해주세요. 
 */
public class Level1_MemoryScore {
    public int[] solution(String[] name, int[] yearning, String[][] photo) {
        //name과 yearning을 Map형태로 저장한다.
        Map<String, Integer> nyMap = new HashMap<>();
        for(int i=0; i<name.length; i++){
            nyMap.put(name[i],yearning[i]);
        }
        //photo의 행의 길이를 구한다.
        int rows_length = photo.length;
        //행의 길이 = 답변의 길이이다.
        int[] answer = new int[rows_length];
        for(int i=0; i<rows_length; i++){
            //각 행에 해당하는 열의 길이를 구한 후
            int column_length = photo[i].length;
            //열의 for문을 돌아 사람에 해당하는 추억 점수를 더한다. 단, 없을 경우에는 0점을 더한다.
            int yearn_score = 0;
            for(int j=0; j<column_length; j++){
                if(nyMap.get(photo[i][j])!=null){
                    yearn_score+=nyMap.get(photo[i][j]);
                }
            }
            answer[i]=yearn_score;
        }
        return answer;
    }
}

/* MEMO>>
 * int arr[][] = [[[],[],[],[]], [[],[]], [[],[],[],[]]]
 * 2차원 배열의 큰 분류(?) 개수는 row(행)이고,
 * 큰 분류 안 소분류는 column(열)이다.
 * 행은 arr.length(=3)로 구하고
 * 열은 arr[i].length(=4,2,4)로 구한다.
 */
